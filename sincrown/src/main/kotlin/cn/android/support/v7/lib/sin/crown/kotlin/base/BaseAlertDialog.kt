...
class BaseAlertDialog(activity: Activity) : BaseDialog(activity, R.layout.crown_dialog_base_alert) {
    ...
    fun Gravity(gravity: Int): BaseAlertDialog {
        this.gravity = gravity
        return this
    }

    fun setContentView(id: Int): BaseAlertDialog {
        val mInflater = LayoutInflater.from(dialog?.context)
        view = mInflater.inflate(id, getParentView(dialog?.window), false)
        return this
    }

    fun setContentView(view: View): BaseAlertDialog {
        this.view = view
    ...
    fun setContentHeight(height: Int): BaseAlertDialog {
        this.height = height
        return this@BaseAlertDialog
    }

    fun builder(): BaseAlertDialog {
        view?.let {
            it?.layoutParams?.let {
                if (width == 0) {
                    width = it?.width!!
